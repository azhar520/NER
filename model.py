...
class Model(nn.Module):
    def __init__(self, config):
        ...
        # self.embedding_perfix = torch.nn.Embedding(config.label_num // 2 + 1, config.lstm_hid_size)
        # self.embedding_postfix = torch.nn.Embedding(config.label_num // 2 + 1, config.lstm_hid_size)
        self.embedding_affix = torch.nn.Embedding(5, config.lstm_hid_size)
        self.use_affix = config.use_affix

    # def forward(self, bert_inputs, grid_mask2d, dist_inputs, pieces2word, sent_length, prefix, postfix):
    def forward(self, bert_inputs, grid_mask2d, dist_inputs, pieces2word, sent_length, affix):
        ...
        # prefix_hidden = self.embedding_perfix(prefix)
        # postfix_hidden = self.embedding_postfix(postfix)
        if self.use_affix:
            affix_hidden = self.embedding_affix(affix)

        # word_reps = (word_reps + prefix_hidden + postfix_hidden) / 3
            word_reps = (word_reps + affix_hidden) / 2
